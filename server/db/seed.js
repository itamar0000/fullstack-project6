...
// Insert the demo dataset, but only into a fresh database. A non-empty users
// table means this database was already initialized, so repeated server starts
...
export async function seed(conn) {
  ...
  for (const t of todos) {
    await conn.execute("INSERT INTO todos (id,userId,title,completed) VALUES (?,?,?,?)", t);
  }
  ...
  for (const p of posts) {
    await conn.execute("INSERT INTO posts (id,userId,title,body) VALUES (?,?,?,?)", p);
  }
  ...
  for (const c of comments) {
    await conn.execute("INSERT INTO comments (id,postId,userId,name,email,body) VALUES (?,?,?,?,?,?)", c);
  }
  ...
  for (const a of albums) {
    await conn.execute("INSERT INTO albums (id,userId,title) VALUES (?,?,?)", a);
  }
  ...
  for (const p of photos) {
    await conn.execute("INSERT INTO photos (id,albumId,title,url,thumbnailUrl) VALUES (?,?,?,?,?)", p);
  }
